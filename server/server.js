...
const corsOptions = {
    origin: function (origin, callback) {
        ...
        if (allowedOrigins.indexOf(origin) !== -1) {
            ...
            // For OAuth providers AND Vercel Preview URLs, allow them
            if (origin && (
                origin.includes('google.com') ||
                origin.includes('facebook.com') ||
                origin.includes('googleapis.com') ||
                origin.endsWith('.vercel.app') // Allow ALL Vercel deployments (prod + previews)
            )) {
                callback(null, true);
            } else {
...
const certificateSettingsSchema = new mongoose.Schema({
    ...
    qrCodeUrl: { type: String, default: 'http://localhost:4173/verify-certificate' }
});
...
const pricingSchema = new mongoose.Schema({
    planType: { type: String, enum: ['free', 'monthly', 'yearly'], required: true, unique: true },
...
app.post('/api/prompt', async (req, res) => {
    ...
    try {
        ...
        const errorMessage = isRateLimitError
            ? 'API rate limit exceeded. Please wait a moment and try again, or check your API quota in Google AI Studio.'
...
//             }
//         ]
//         Where correctAnswer is the index (0-3) of the correct option. Only return the JSON array, nothing else.`;
...
//             : 'Failed to generate quiz: ' + (error.message || String(error));
...
app.get('/api/shareable', async (req, res) => {
    try {
        const { id } = req.query;
        // Use lean() for read-only queries and add indexes
        const result = await Course.find({ _id: id }).lean();

        // Set cache headers for this static course data (24 hours)
...
app.get('/api/getcourses', async (req, res) => {
    try {
        // Only fetch summary fields, not the large content field
        const courses = await Course.find({})
            .select('_id user mainTopic type photo date end completed restricted')
            .lean()
            .sort({ date: -1 });

        // Set cache headers (1 hour for admin listing)
...
app.post('/api/chat', async (req, res) => {
    ...
    // Use the same stable model name used elsewhere and improve error visibility
    const model = genAI.getGenerativeModel({ model: "gemini-flash-latest", safetySettings });
    ...
    await model.generateContent(prompt).then(async result => {
        ...
        if (process.env.NODE_ENV === 'development') {
            res.status(500).json({ success: false, message: error?.message || 'Internal server error', details: error });
        } else {
            res.status(500).json({ success: false, message: 'Internal server error' });
        }
    })
...
// GET PRICING (PUBLIC) - Fetch all pricing plans for frontend
    ...
    try {
        ...
        if (pricingPlans.length === 0) {
            ...
            const defaultPricing = [
                { planType: 'free', planName: 'Free Plan', price: 0, currency: 'USD', billingPeriod: 'Lifetime' },
                { planType: 'monthly', planName: 'Monthly Plan', price: 9, currency: 'USD', billingPeriod: 'Month' },
                { planType: 'yearly', planName: 'Yearly Plan', price: 99, currency: 'USD', billingPeriod: 'Year' }
            ];
            await Pricing.insertMany(defaultPricing);
            return res.json({ success: true, pricing: defaultPricing });
        }

        // Return as array for frontend
        res.json({ success: true, pricing: pricingPlans });
    } catch (error) {
...
// GET PRICING - Fetch all pricing plans
app.get('/api/admin/pricing', async (req, res) => {
    try {
        ...
        if (pricingPlans.length === 0) {
            ...
            // Convert to object format before returning
            const pricingObj = {};
            ...
            return res.json({ success: true, pricing: pricingObj });
        }
        ...
        res.json({ success: true, pricing: pricingObj });
    } catch (error) {
        console.log('Error fetching pricing:', error);
        res.status(500).json({ success: false, message: 'Internal server error' });
    }
...
// UPDATE PRICING - Update pricing plans
app.post('/api/admin/pricing/update', async (req, res) => {
    try {
        const { pricing } = req.body;

        if (!pricing || typeof pricing !== 'object') {
            return res.json({ success: false, message: 'Invalid pricing data' });
        }

        // Update or create each pricing plan
...
// GET TESTIMONIALS (PUBLIC) - Fetch all approved testimonials
...
// POST TESTIMONIAL - Customer submits a testimonial
app.post('/api/testimonials/submit', async (req, res) => {
    try {
        const { userEmail, userName, message, rating, profession, photoUrl } = req.body;
        ...
        res.json({ success: true, message: 'Testimonial submitted successfully and is pending admin approval' });
    } catch (error) {
...
// GET TESTIMONIALS (ADMIN) - Fetch all testimonials for admin approval
...
// UPDATE TESTIMONIAL (ADMIN) - Approve/Reject/Feature testimonial
app.post('/api/admin/testimonials/:id/update', async (req, res) => {
    try {
        const { id } = req.params;
...
// DELETE TESTIMONIAL (ADMIN) - Delete a testimonial
...
// Serve static files from the React app only if NOT in Vercel environment
